...
//    printf("m_frames,m_size,m_frameNum: %d,  %d\n",m_frames.size(),m_frameNum);
		...
		if (k >= 0){
			...
			for (int i = 0; i < m_height; i++){
				for (int j = 0; j < m_width; j++){
					...
					int a2 = abs(m_frames[reference].at<cv::Vec3b>(i, j)[0] - m_temp.at<cv::Vec3b>(i, j)[0]);
					int a3 = abs(m_frames[reference].at<cv::Vec3b>(i, j)[2] - m_temp.at<cv::Vec3b>(i, j)[2]);
					float b = 0;
//					a > 10 ? b = 0 : b = 1;
//                    (a+a2+a3)*0.33 > 10 ? b = 0 : b = 1;
					(0.5*a+0.25*a2+0.25*a3) > 10 ? b = 0 : b = 1;
...
//	outVideoWriter.open(name, CV_FOURCC('X', 'V', 'I', 'D'), m_fps,m_size);
	outVideoWriter.open(name, CV_FOURCC('M', 'J', 'P', 'G'), m_fps,m_size);    
	
	for (int i = 0; i < m_frameNum; i++){
        printf("i: %d/n",i);
        char name[512];
	   sprintf(name, "/Users/itoyuichi/github/playground/OpenCV/stab-opencv/meshflow/%04d.jpg",i);
        imwrite(name,dst[i]);
